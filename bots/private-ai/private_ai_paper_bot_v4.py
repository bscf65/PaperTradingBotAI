#!/usr/bin/env python3
"""
AI Private-Tech Paper Investment Bot v4

Paper-only scanner/simulator for AI/private-technology access funds, AI ETFs, and tech proxies.
No broker connection. No live orders.

Default universe includes:
- VCX, DXYZ
- ARKK, ARKQ, QQQ, SMH, AIQ
...
- News advisory via GDELT, Briefs/Market Briefs, and global source feeds (optional)
...
STATE_FILE = LOG_DIR / f"private_ai_state_{VERSION}.json"
...
DEFAULT_UNIVERSE = ["VCX", "DXYZ", "ARKK", "ARKQ", "QQQ", "SMH", "AIQ"]
DEFAULT_ALIASES = {"FUNDRISE": "VCX", "DESTINY": "DXYZ"}
ETF_SYMBOLS = {"ARKK", "ARKQ", "QQQ", "SMH", "AIQ", "SPY", "XLK"}

BRIEFS_CATEGORY_URLS = {
    "stock": "https://www.briefs.co/all-news/stock/",
    "technology": "https://www.briefs.co/all-news/technology/",
    "crypto": "https://www.briefs.co/all-news/crypto/",
    "economy": "https://www.briefs.co/all-news/economy/",
}

BRIEFS_RISK_TERMS = [
    "lawsuit", "SEC", "fraud", "dilution", "offering", "bankruptcy",
    "hack", "short seller", "investigation", "missed", "warning",
    "layoffs", "debt", "downgrade", "plunge", "falls", "slides",
]

BRIEFS_POSITIVE_TERMS = [
    "beats", "surged", "jumps", "record", "raises", "profit", "partnership",
    "funding", "contract", "launch", "approved", "upgrade", "growth",
]


GLOBAL_NEWS_FEEDS = {
    "bbc_business": {"url": "https://feeds.bbci.co.uk/news/business/rss.xml", "weight": 0.95, "bucket": "mainstream"},
    "bbc_world": {"url": "https://feeds.bbci.co.uk/news/world/rss.xml", "weight": 0.90, "bucket": "mainstream"},
    "aljazeera_all": {"url": "https://www.aljazeera.com/xml/rss/all.xml", "weight": 0.85, "bucket": "mainstream"},
    "india_today_latest": {"url": "https://www.indiatoday.in/rss/latest.xml", "weight": 0.75, "bucket": "india"},
    "india_today_business": {"url": "https://www.indiatoday.in/rss/1206574", "weight": 0.75, "bucket": "india"},
    "cnbc_top": {"url": "https://www.cnbc.com/id/100003114/device/rss/rss.html", "weight": 0.85, "bucket": "markets"},
    "marketwatch_top": {"url": "https://feeds.content.dowjones.io/public/rss/mw_topstories", "weight": 0.80, "bucket": "markets"},
    "cgtn_world": {"url": "https://news.cgtn.com/news/rss/world.xml", "weight": 0.35, "bucket": "china_state_media"},
}

GLOBAL_NEWS_RISK_TERMS = [
    "tariff", "tariffs", "sanction", "sanctions", "export control", "chip ban", "blacklist",
    "crackdown", "probe", "investigation", "lawsuit", "fraud", "default", "debt crisis",
    "rate hike", "inflation", "selloff", "war", "conflict", "taiwan", "delisting",
    "restriction", "ban", "misses", "downgrade", "plunge", "slump", "bubble",
]

GLOBAL_NEWS_POSITIVE_TERMS = [
    "partnership", "contract", "funding", "approval", "approved", "upgrade", "beats",
    "rate cut", "stimulus", "investment", "launch", "record", "surge", "jumps", "growth",
    "chip demand", "ai demand", "data center", "breakthrough", "deal", "expands",
]

BRICS_TERMS = ["brics", "dedollarization", "de-dollarization", "yuan", "renminbi", "local currency", "swift", "tariff", "trade bloc", "russia", "india", "china", "brazil", "south africa"]
CHINA_TERMS = ["china", "beijing", "xi", "xinhua", "cgtn", "chip export", "export controls", "taiwan", "semiconductor", "rare earth", "manufacturing", "stimulus"]
BLOG_LOW_TRUST_TERMS = ["blog", "rumor", "vlog", "youtube", "substack", "reddit", "x.com", "twitter", "weibo", "telegram"]
...
def load_state(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if args.reset:
        for path in [STATE_FILE, TRADES_FILE, EQUITY_FILE, SCAN_FILE, NEWS_FILE]:
            if path.exists():
                path.unlink()
        print("Reset complete: deleted v4 private-AI simulation state/log files.")
...
class BriefsHeadlineParser(HTMLParser):
    """Tiny stdlib HTML parser that extracts link text from Briefs category pages."""
    ...
    def handle_data(self, data: str) -> None:
        if self.in_link:
            ...
            if clean:
                self.current_parts.append(clean)
...
def query_terms(symbol: str, query: str) -> List[str]:
    """Return flexible matching terms for advisory news filtering."""
    base = [symbol, *re.findall(r"[A-Za-z][A-Za-z0-9]{2,}", query)]
    extras = ["AI", "OpenAI", "Anthropic", "SpaceX", "Databricks", "semiconductor", "chip"]
    seen = set()
    terms: List[str] = []
...
def fetch_gdelt_news(symbol: str, query: str) -> Dict[str, Any]:
    ...
    try:
        r = requests.get(url, timeout=10, headers={"User-Agent": "InvestAI-paper-bot/1.0"})
        if not r.ok:
            return {"count": 0, "risk_hits": 0, "positive_hits": 0, "headlines": [f"gdelt_http_{r.status_code}"]}
        data = r.json()
        ...
        for a in articles[:5]:
            ...
            low = title.lower()
            risk_hits += sum(1 for term in BRIEFS_RISK_TERMS if term.lower() in low)
            positive_hits += sum(1 for term in BRIEFS_POSITIVE_TERMS if term.lower() in low)
        return {"count": len(articles), "risk_hits": risk_hits, "positive_hits": positive_hits, "headlines": headlines[:3]}
    except Exception as exc:
        return {"count": 0, "risk_hits": 0, "positive_hits": 0, "headlines": [f"gdelt_error: {exc}"]}


def fetch_briefs_news(symbol: str, query: str, args: Optional[argparse.Namespace] = None) -> Dict[str, Any]:
    """Scrape public Briefs/Market Briefs category headlines for advisory context only.

    This deliberately reads only category-page headline/link text. It does not use Briefs
    Pro, does not log in, and does not trade directly from headlines.
    """
    sources = getattr(args, "briefs_categories", "stock,technology,crypto,economy") if args else "stock,technology,crypto,economy"
    max_headlines = int(getattr(args, "briefs_max_headlines", 40) if args else 40)
    wanted_categories = [x.strip().lower() for x in str(sources).split(",") if x.strip()]
    terms = [t.lower() for t in query_terms(symbol, query)]
    headlines: List[str] = []
    all_count = 0

    for category in wanted_categories:
        url = BRIEFS_CATEGORY_URLS.get(category)
        if not url:
            continue
        try:
            r = requests.get(url, timeout=10, headers={"User-Agent": "InvestAI-paper-bot/1.0"})
            if not r.ok:
                headlines.append(f"briefs_{category}_http_{r.status_code}")
                continue
            parser = BriefsHeadlineParser()
            parser.feed(r.text)
            for href, text in parser.links:
                ...
                if lower in {"stock", "crypto", "technology", "economy", "read more", "next »"}:
                    continue
                if any(nav in lower for nav in ["pro login", "free newsletter", "subscribe", "close menu", "briefs finance"]):
                    continue
                all_count += 1
                if any(term in lower for term in terms):
                    headlines.append(f"{category}: {text_clean}")
                if len(headlines) >= max_headlines:
                    break
        except Exception as exc:
            headlines.append(f"briefs_{category}_error: {exc}")
        if len(headlines) >= max_headlines:
            break

    matched = headlines[:max_headlines]
    risk_hits = 0
    positive_hits = 0
...
def parse_rss_items(xml_text: str, max_items: int = 25) -> List[Dict[str, str]]:
    """Parse RSS/Atom-ish feeds using only the stdlib. Best effort."""
    ...
    for item in root.findall(".//item"):
        ...
        if title:
            items.append({"title": " ".join(title.split()), "description": " ".join(desc.split()), "link": link})
...
def fetch_global_feed_news(symbol: str, query: str, args: Optional[argparse.Namespace] = None) -> Dict[str, Any]:
    """Fetch advisory headlines from selected global sources. News never directly forces trades.

    Mainstream sources get higher weight. China/state-aligned and blog-like sources are logged with lower
    confidence so we can watch BRICS/China narratives without treating them as hard signals.
    """
    requested = getattr(args, "global_news_feeds", "bbc_business,bbc_world,aljazeera_all,india_today_latest,india_today_business,cnbc_top,marketwatch_top,cgtn_world") if args else "bbc_business,bbc_world,aljazeera_all,india_today_latest,india_today_business,cnbc_top,marketwatch_top,cgtn_world"
    max_items = int(getattr(args, "global_news_max_items", 80) if args else 80)
    watch_china_brics = bool(getattr(args, "watch_china_brics", True) if args else True)
    selected = [x.strip().lower() for x in str(requested).split(",") if x.strip()]
    terms = [t.lower() for t in query_terms(symbol, query)]
    if watch_china_brics:
        terms += [t.lower() for t in CHINA_TERMS + BRICS_TERMS]
    ...
    for name in selected:
        meta = GLOBAL_NEWS_FEEDS.get(name)
        if not meta:
            continue
        url = str(meta["url"])
        weight = safe_float(meta.get("weight", 0.5), 0.5)
        bucket = str(meta.get("bucket", "unknown"))
        source_buckets[name] = bucket
        try:
            r = requests.get(url, timeout=10, headers={"User-Agent": "InvestAI-paper-bot/1.0"})
            if not r.ok:
                headlines.append(f"{name}_http_{r.status_code}")
                continue
            items = parse_rss_items(r.text, max_items=max_items)
        except Exception as exc:
            headlines.append(f"{name}_error: {exc}")
            continue
        ...
        for item in items:
            ...
            matched = any(term and term in lower for term in terms)
            china = sum(1 for term in CHINA_TERMS if term.lower() in lower)
            brics = sum(1 for term in BRICS_TERMS if term.lower() in lower)
            risk = sum(1 for term in GLOBAL_NEWS_RISK_TERMS + BRIEFS_RISK_TERMS if term.lower() in lower)
            positive = sum(1 for term in GLOBAL_NEWS_POSITIVE_TERMS + BRIEFS_POSITIVE_TERMS if term.lower() in lower)
            low_trust = sum(1 for term in BLOG_LOW_TRUST_TERMS if term.lower() in lower)
            if matched or china or brics:
                count += 1
                ...
                china_hits += china
                brics_hits += brics
                low_trust_hits += low_trust
                impact = (positive * 1.0) - (risk * 1.25) - (brics * 0.25) - (china * 0.15)
                if low_trust:
                    impact *= 0.50
                weighted_impact += impact * weight
                if len(headlines) < 8:
                    trust_note = "LOW_WEIGHT" if weight < 0.5 or low_trust else ""
                    headlines.append(f"{name}/{bucket}{('/' + trust_note) if trust_note else ''}: {title}")
    ...
    return {
        ...
        "brics_hits": brics_hits,
        "low_trust_hits": low_trust_hits,
        "weighted_impact": round(weighted_impact, 3),
        "source_breakdown": ";".join(f"{k}:{v}" for k, v in sorted(source_counts.items())),
        "source_buckets": ";".join(f"{k}:{v}" for k, v in sorted(source_buckets.items())),
        "headlines": headlines[:5],
    }


def fetch_news(symbol: str, query: str, args: Optional[argparse.Namespace] = None) -> Dict[str, Any]:
    """Combined advisory news fetcher. News never directly forces trades."""
    sources_raw = getattr(args, "news_sources", "gdelt,briefs,globalfeeds") if args else "gdelt,briefs,globalfeeds"
    sources = [s.strip().lower() for s in str(sources_raw).split(",") if s.strip()]

    gdelt = {"count": 0, "risk_hits": 0, "positive_hits": 0, "headlines": []}
    briefs = {"count": 0, "scanned_count": 0, "risk_hits": 0, "positive_hits": 0, "headlines": []}
    globalfeeds = {"count": 0, "scanned_count": 0, "risk_hits": 0, "positive_hits": 0, "china_hits": 0, "brics_hits": 0, "low_trust_hits": 0, "weighted_impact": 0.0, "source_breakdown": "", "source_buckets": "", "headlines": []}

    if "gdelt" in sources:
        gdelt = fetch_gdelt_news(symbol, query)
    if "briefs" in sources or "marketbriefs" in sources or "market_briefs" in sources:
        briefs = fetch_briefs_news(symbol, query, args)
    if "globalfeeds" in sources or "global" in sources or "rss" in sources:
        globalfeeds = fetch_global_feed_news(symbol, query, args)

    headline_sample = []
    for src_name, payload in [("GDELT", gdelt), ("Briefs", briefs), ("Global", globalfeeds)]:
        for h in payload.get("headlines", [])[:3]:
            headline_sample.append(f"{src_name}: {h}")

    risk_hits = int(gdelt.get("risk_hits", 0)) + int(briefs.get("risk_hits", 0)) + int(globalfeeds.get("risk_hits", 0))
    positive_hits = int(gdelt.get("positive_hits", 0)) + int(briefs.get("positive_hits", 0)) + int(globalfeeds.get("positive_hits", 0))
    article_count = int(gdelt.get("count", 0)) + int(briefs.get("count", 0)) + int(globalfeeds.get("count", 0))
    news_impact_score = round((positive_hits * 1.0) - (risk_hits * 1.25) + safe_float(globalfeeds.get("weighted_impact", 0.0), 0.0), 3)

    return {
        "symbol": symbol,
        "sources": ",".join(sources),
        "article_count": article_count,
        "gdelt_article_count": int(gdelt.get("count", 0)),
        "briefs_article_count": int(briefs.get("count", 0)),
        "briefs_scanned_count": int(briefs.get("scanned_count", 0)),
        "global_article_count": int(globalfeeds.get("count", 0)),
        "global_scanned_count": int(globalfeeds.get("scanned_count", 0)),
        "risk_hits": risk_hits,
        "positive_hits": positive_hits,
        "china_hits": int(globalfeeds.get("china_hits", 0)),
        "brics_hits": int(globalfeeds.get("brics_hits", 0)),
        "low_trust_hits": int(globalfeeds.get("low_trust_hits", 0)),
        "global_weighted_impact": safe_float(globalfeeds.get("weighted_impact", 0.0), 0.0),
        "news_impact_score": news_impact_score,
        "source_breakdown": globalfeeds.get("source_breakdown", ""),
        "source_buckets": globalfeeds.get("source_buckets", ""),
        "headline_sample": " | ".join(headline_sample[:8]),
    }
...
def log_news(row: Dict[str, Any], args: argparse.Namespace) -> None:
    header = [
        "timestamp_local", "timestamp_utc", "run_label", "symbol", "sources",
        "article_count", "gdelt_article_count", "briefs_article_count", "briefs_scanned_count",
        "global_article_count", "global_scanned_count", "risk_hits", "positive_hits",
        "china_hits", "brics_hits", "low_trust_hits", "global_weighted_impact",
        "news_impact_score", "source_breakdown", "source_buckets", "headline_sample"
    ]
    row = {**row, "timestamp_local": local_iso(), "timestamp_utc": utc_iso(), "run_label": args.run_label}
...
def run_cycle(state: Dict[str, Any], args: argparse.Namespace) -> None:
    ...
    print("\n" + "=" * 102)
    print(f"AI PRIVATE-TECH PAPER INVESTMENT BOT {VERSION} - Local {local_iso()} | UTC {utc_iso()}")
    print("=" * 102)
    print(f"Run label: {args.run_label} | Cash: {money(safe_float(state.get('cash', 0.0), 0.0))} | Symbols: {', '.join(symbols)}")
    print(f"Mode: stocks={args.enable_equities} options={args.enable_options} news={args.enable_news} sources={getattr(args, 'news_sources', 'gdelt,briefs')} scan_only={args.scan_only}")
    if notes:
        for n in notes:
    ...
    if args.enable_news:
        for scan in scans:
            ...
            row = fetch_news(scan["symbol"], q, args)
            log_news(row, args)
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="AI Private-Tech paper investment bot: stocks, ETFs, and options.")
    p.add_argument("--config", default=None, help="Path to JSON config file.")
    p.add_argument("--run-label", default="private_ai_v2", help="Label included in logs.")
    ...
    p.add_argument("--news-sources", default="gdelt,briefs,globalfeeds", help="Comma-separated advisory news sources: gdelt,briefs,globalfeeds")
    p.add_argument("--briefs-categories", default="stock,technology,crypto,economy", help="Briefs category pages to scan, comma-separated.")
    p.add_argument("--briefs-max-headlines", type=int, default=40, help="Max matching Briefs headlines to evaluate per symbol.")
    p.add_argument("--global-news-feeds", default="bbc_business,bbc_world,aljazeera_all,india_today_latest,india_today_business,cnbc_top,marketwatch_top,cgtn_world", help="Comma-separated global RSS/feed sources to scan.")
    p.add_argument("--global-news-max-items", type=int, default=80, help="Max total global feed items to scan per symbol per cycle.")
    p.add_argument("--watch-china-brics", action=argparse.BooleanOptionalAction, default=True, help="Track China/BRICS narrative terms with source weighting. Advisory only.")
...
def main() -> int:
    ...
    if args.once:
        ...
        return 0
    print(f"Starting AI Private-Tech Paper Bot {VERSION}. Press Ctrl+C to stop.")
