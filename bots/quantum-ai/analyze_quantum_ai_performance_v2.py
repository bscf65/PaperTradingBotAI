#!/usr/bin/env python3
"""Analyze Quantum/AI Paper Bot v2 logs."""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Analyze Quantum/AI Paper Bot v2 logs.")
    ap.add_argument("--log-dir", default=str(LOG_DIR))
    args = ap.parse_args()

    log_dir = Path(args.log_dir).expanduser().resolve()
    trades = read_csv(log_dir / f"quantum_ai_trades_{VERSION}.csv")
    equity = read_csv(log_dir / f"quantum_ai_equity_{VERSION}.csv")
    scans = read_csv(log_dir / f"quantum_ai_scan_{VERSION}.csv")
    news = read_csv(log_dir / f"quantum_ai_news_{VERSION}.csv")

    print("=" * 80)
    print("QUANTUM/AI PAPER BOT PERFORMANCE ANALYZER v2")
    print("=" * 80)
